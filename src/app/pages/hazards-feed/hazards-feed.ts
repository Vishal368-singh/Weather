...
export class HazardsFeed implements OnInit, AfterViewInit {
  ...
  tableData: any[] = [];
  triggerData: any[] = [];
  triggerPopupModel: boolean = false;
  currenttableData: any[] = [];
  snowfallDistrictList: any = [];
  isSwitch: boolean = false;
  currentAPI: string | null = null;
  switchAPI: string | null = null;
  ...
  async getTriggersLog() {
    try {
      const res: any = await this.dataService
        .postRequest('get_triggers_log', {})
        .toPromise();
      this.triggerData = [];
      this.ngZone.run(() => {
        if (res && res.status && Array.isArray(res.data)) {
          const data = res.data.map((ele: any) => ({
            id: ele.id,
            trigger_name: ele.trigger_name,
            trigger_purpose: ele.trigger_purpose,
            internal_external: ele.internal_external,
            org: ele.org,
            aws: ele.aws,
            biz: ele.biz,
            execution_status: ele.execution_status,
            execution_time: ele.execution_time,
            last_update: ele.last_update,
          }));

          this.triggerData = [...data];
          this.triggerPopupModel = true;

          // OnPush: trigger UI update
          this.cdr.markForCheck();
        } else {
          console.error(
            'Failed to load triggers : Invalid API response format',
          );
          this.triggerData = [];
          this.cdr.markForCheck();
        }
      ...
      console.error('Failed to load triggers from API:', error);
  ...
  requestToSwitchAPI() {
    ...
    this.currentAPI = localStorage.getItem('service_type');
    this.switchAPI =
      this.currentAPI === 'weatherapi' ? 'crossvisualapi' : 'weatherapi';
    this.isSwitch = true;
    this.cdr.markForCheck();
  }
  async SwitchApiToggle() {
    this.dataService
      .postRequest('get_service', { switch: 'switch' })
      .subscribe((res: any) => {
        
        localStorage.setItem('service_type', res.data);
        this.snackBar.open('API switched successfully', 'X', {
          duration: 2000, // auto close after 3s
          horizontalPosition: 'center',
          verticalPosition: 'bottom',
          panelClass: ['custom-success-snackbar'],
