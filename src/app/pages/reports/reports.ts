...
export class Reports implements OnInit {
  ...
  selectedDateHourlyReport = this.formatShortDate(new Date());
  ...
  /* Heading For 7 -dfc table (Daily Report)*/
  headingMap: Record<string, string> = {
    temperature:
      'Temparture Departure/Coldwave-Hotwave Forecast - 7 Days (W.r.t to Normal) (°C)',
  ...
  /* Heading For 7 -dfc table (Hourly Report) */
  headingMapHourly: Record<string, string> = {
    temperature:
      'Temparture Departure/Coldwave-Hotwave Forecast - Hourly (W.r.t to Normal) (°C)',
    rainfall: 'Rainfall Forecast - Hourly (mm)',
    accu_rainfall: 'Accumulated Rainfall Forecast - Hourly (mm)',
    humidity: 'Humidity Forecast - Hourly (%)',
    wind: 'Wind Forecast - Hourly (kmph)',
    visibility: 'Visibility  Forecast - Hourly (km)',
  };
  ...
  headingMapHaz: Record<string, string> = {
    ...
    Cloudburst: 'Cloudburst Forecast - 7 Days (%)',
    Lightning: 'Lightning Occurrence Probability Forecast - 7 Days',
    Landslide: 'Landslide Forecast - 7 Days (%)',
  ...
  //#region Weather Parameter Breakdown
  ...
  fetch7daysForecastData() {
    ...
    this.dataService
      ...
      .subscribe((response: any) => {
        ...
        //  CRITICAL ADDITION: If severity ranges are already loaded, run the chart/table logic now.
  ...
  //#region Today's Risk Intensity of Districts for Weather Parameters
  ...
  //#region Today's Risk Intensity of Districts for Hazard
      ...
      .subscribe({
        next: (response: any) => {
          ...
          this.responseDataDistrictWise = kpidata.map((ele: any) => ({
            ...
            day1: ele.acc_1_to_3.toFixed(2),
            day2: ele.acc_2_to_4.toFixed(2),
            day3: ele.acc_3_to_5.toFixed(2),
  ...
  selectDate(d: any) {
    this.selectedDateHourlyReport = d;
    this.safeDetectChanges;
  }
  //#endregion
}
